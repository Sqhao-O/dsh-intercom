...
test("normalizeCwd falls back to the resolved path for a nonexistent dir", () => {
  ...
  // lexical variants rather than throw. Compare against resolve() so the
  // expectation is platform-correct (drive letters/backslashes on Windows).
  assert.equal(normalizeCwd("/no/such/dir/../dir"), resolve("/no/such/dir"));
});
