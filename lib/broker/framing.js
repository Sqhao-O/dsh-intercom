...
function createMessageReader(onMessage, onError, maxFrameBytes = MAX_FRAME_BYTES) {
	...
	let payloadBytes = 0;
	let payloadLength = 0;
	function reportMessage(framePayload) {
		let msg;
		try {
		...
		while (offset < data.length) {
			if (headerBytes < 4) {
				...
				if (payloadLength > maxFrameBytes) {
					headerBytes = 0;
					onError(/* @__PURE__ */ new Error(`Intercom frame length ${payloadLength} exceeds maximum ${maxFrameBytes} bytes`));
					return;
				}
			}
			...
			if (payload === null || payload.length !== payloadLength) payload = Buffer.allocUnsafe(payloadLength);
...
//#endregion
export { createMessageReader, writeMessage };
