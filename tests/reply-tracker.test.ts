/**
 * Ported from pi-intercom's `reply-tracker.test.ts` (see NOTICE): the
 * environment variable moved to `DSH_INTERCOM_ASK_TIMEOUT_MS`, and the
 * multiple-pending-asks error now points at the `pending` action.
 */
import test from "node:test";
import assert from "node:assert/strict";
import { getAskTimeoutMs } from "../broker/ask-timeout.ts";
import { ReplyTracker } from "../src/reply-tracker.ts";
...
test("activateTurnContext adopts a claimed relay as the reply target", () => {
  const tracker = new ReplyTracker();
  ...
  tracker.activateTurnContext("ask-2", 1002);

  assert.equal(tracker.resolveReplyTarget({}, 1003).message.id, "ask-2");
  tracker.endTurn();
  assert.throws(
    () => tracker.resolveReplyTarget({}, 1004),
    /Multiple pending asks/,
  );
});

test("activateTurnContext ignores unknown or expired message ids", () => {
  ...
  tracker.activateTurnContext("nope", 1001);
  assert.equal(tracker.resolveReplyTarget({}, 1002).message.id, "ask-1");

  tracker.activateTurnContext("ask-1", 1000 + 11);
  assert.throws(
    () => tracker.resolveReplyTarget({}, 1012),
...
test("reply with to resolves matching pending ask", () => {
  ...
  assert.equal(
    tracker.resolveReplyTarget({ to: "ReViEwEr" }, 1002).message.id,
    "ask-2",
  );
  assert.equal(
    tracker.resolveReplyTarget({ to: "planner-id" }, 1002).message.id,
    "ask-1",
  );
});
...
test("reply with to resolves a pending ask by a unique short session ID", () => {
  const tracker = new ReplyTracker();
  tracker.recordIncomingMessage(
    createSession("019fd3c4-1111-7222-8333-444444444444", "planner"),
    createMessage("ask-1", "First"),
    1000,
  );
  tracker.recordIncomingMessage(
    createSession("019fd3d5-1111-7222-8333-444444444444", "reviewer"),
    createMessage("ask-2", "Second"),
    1001,
  );

  assert.equal(
    tracker.resolveReplyTarget({ to: "019fd3c4" }, 1002).message.id,
    "ask-1",
  );
  assert.equal(
    tracker.resolveReplyTarget({ to: "019fd3c4", replyTo: "ask-1" }, 1002)
      .message.id,
    "ask-1",
  );
});
...
test("reply with to prefers an exact sender name over another sender ID prefix", () => {
  const tracker = new ReplyTracker();
  tracker.recordIncomingMessage(
    createSession("planner-session-id", "019fd3c4"),
    createMessage("ask-1", "Named sender"),
    1000,
  );
  tracker.recordIncomingMessage(
    createSession("019fd3c4-1111-7222-8333-444444444444", "reviewer"),
    createMessage("ask-2", "Prefixed sender"),
    1001,
  );
...
test("reply with to clearly rejects an ambiguous session ID prefix", () => {
  ...
  tracker.recordIncomingMessage(
    createSession("019fd3c4-5555-7666-8777-888888888888", "reviewer"),
    createMessage("ask-2", "Second"),
    1001,
  );
...
test("explicit to overrides the current turn context", () => {
  ...
  assert.equal(
    tracker.resolveReplyTarget({ to: "reviewer" }, 1003).message.id,
    "ask-2",
  );
  assert.throws(
    () => tracker.resolveReplyTarget({ to: "missing" }, 1003),
    /No pending ask from/,
  );
});
...
test("replyTo resolves the exact pending ask", () => {
  ...
  assert.equal(
    tracker.resolveReplyTarget({ replyTo: "ask-2" }, 1002).from.id,
    "reviewer-id",
  );
  assert.throws(
    () => tracker.resolveReplyTarget({ to: "planner", replyTo: "ask-2" }, 1002),
    /is not from/,
  );
});
...
test("reply errors when multiple pending asks and no to, pointing at pending", () => {
  const tracker = new ReplyTracker();
  tracker.recordIncomingMessage(
    createSession("planner-id", "planner"),
    createMessage("ask-1", "First"),
  ...
  assert.throws(
    ...
    /Multiple pending asks — list them with intercom\(\{ action: "pending" \}\) and specify `to`/,
  );
});
...
test("ask timeout can be configured from environment", () => {
  const previous = process.env.DSH_INTERCOM_ASK_TIMEOUT_MS;
  process.env.DSH_INTERCOM_ASK_TIMEOUT_MS = "42";
  try {
    assert.equal(getAskTimeoutMs(), 42);
    assert.throws(() => {
      process.env.DSH_INTERCOM_ASK_TIMEOUT_MS = "0";
      getAskTimeoutMs();
    }, /positive integer/);
  } finally {
    if (previous === undefined) delete process.env.DSH_INTERCOM_ASK_TIMEOUT_MS;
    else process.env.DSH_INTERCOM_ASK_TIMEOUT_MS = previous;
  }
});
...
test("pending asks can be explicitly dismissed without removing retryable failures", () => {
  ...
  assert.deepEqual(
    tracker.listPending(1002).map((context) => context.message.id),
    ["ask-2"],
  );
});
...
test("findUniquePendingAskFrom does not resolve an ID prefix", () => {
  const tracker = new ReplyTracker();
  tracker.recordIncomingMessage(
    createSession("abcdef-session-id", "planner"),
    createMessage("ask-1", "Need a decision"),
    1000,
  );
