...
test(
  "extension bus negotiates, routes, elects an owner, and persists state",
  { concurrency: false, timeout: 30_000 },
  async () => {
    const agentDir = mkdtempSync(
      path.join(tmpdir(), "dsh-intercom-extension-"),
    );
    const previousAgentDir = process.env.DSH_HOME;
    process.env.DSH_HOME = agentDir;
    ...
    try {
      ...
      await assert.rejects(
        invalidReplacement.connect(
          {
            ...registration("invalid-replacement", now),
            extensions: [
              { namespace: "Invalid Namespace", ownerEligible: true },
            ],
          },
          "owner-id",
        ),
      ...
      assert.equal(
        legacy.supportsFeature("extension-bus-v1"),
        true,
        "new broker advertises support even to clients without capabilities",
      );

      const ownerEvent = await waitFor(
        ownerMessages,
        (message) => message.type === "extension_owner",
      );
      assert.equal(ownerEvent.type, "extension_owner");
      assert.equal(ownerEvent.ownerId, "owner-id");
      assert.ok(ownerEvent.ownerEpoch);
      const peerOwnerEvent = await waitFor(
        peerMessages,
        (message) => message.type === "extension_owner",
      );
      ...
      late.updateExtensionCapabilities([
        { namespace: "test/v1", ownerEligible: true },
      ]);
      const lateOwnerEvent = await waitFor(
        lateMessages,
        (message) => message.type === "extension_owner",
      );
      assert.equal(lateOwnerEvent.type, "extension_owner");
      assert.equal(
        lateOwnerEvent.ownerId,
        "owner-id",
        "backdated client must not seize namespace ownership",
      );
      late.updateExtensionCapabilities([
        { namespace: "test/v1", ownerEligible: false },
      ]);

      legacy.updateExtensionCapabilities([
        { namespace: "other/v1", ownerEligible: false },
      ]);
      ...
      assert.match(
        legacyErrors[0]?.message ?? "",
        /does not have capability for this namespace/,
      );
      legacy.sendExtensionMessage({
      ...
      while (
        Date.now() < oversizedDeadline &&
        !ownerErrors.some((error) => /16 KiB/.test(error.message))
      ) {
        await new Promise((resolve) => setTimeout(resolve, 10));
      }
      assert.equal(
      ...
      const peerBroadcast = await waitFor(
        peerOnlyBMessages,
      ...
      const ownerTarget = await waitFor(
        ownerMessages,
        (message) =>
          message.type === "extension_message" &&
      ...
      const lateReceived = await waitFor(
        lateMessages,
        (message) => message.type === "extension_message",
      );
      ...
      await new Promise((resolve) => setTimeout(resolve, 50));
      assert.equal(
        legacyMessages.some((message) => message.type === "extension_message"),
        false,
      );
      ...
      const state = await waitFor(
        peerMessages,
        (message) => message.type === "extension_state",
      );
      ...
      while (
        Date.now() < missingEpochDeadline &&
        !ownerErrors.some((error) => /ownerEpoch required/.test(error.message))
      ) {
        await new Promise((resolve) => setTimeout(resolve, 10));
      }
      assert.equal(
      ...
      await waitFor(
        ...
        (message) =>
          message.type === "error" &&
          message.error === "Owner validation failed",
      ).catch(async () => {
      ...
      const noOwner = await waitFor(
        peerMessages,
        (message) =>
          message.type === "extension_owner" && message.ownerId === undefined,
      );
      ...
      replacement.onBrokerMessage((message) =>
        replacementMessages.push(message),
      );
      replacement.on("error", () => {});
      await replacement.connect(
        registration("replacement", now + 1, true),
        "owner-id",
      );
      const replacementOwner = await waitFor(
        replacementMessages,
        (message) => message.type === "extension_owner",
      );
      ...
      if (previousAgentDir === undefined) delete process.env.DSH_HOME;
      else process.env.DSH_HOME = previousAgentDir;
...
test("extension state compare-and-swap rejects stale and invalid revisions", () => {
  const runtimeDir = mkdtempSync(
    path.join(tmpdir(), "dsh-intercom-state-cas-"),
  );
  try {
    const manager = new ExtensionStateManager(runtimeDir);
    assert.deepEqual(manager.commitState("test/v1", 0, { version: 1 }), {
      committed: true,
      revision: 1,
    });
...
test("extension state falls back to a valid backup", () => {
  const runtimeDir = mkdtempSync(path.join(tmpdir(), "dsh-intercom-state-"));
  try {
    const manager = new ExtensionStateManager(runtimeDir);
    assert.equal(
      manager.commitState("test/v1", 0, { version: 1 }).committed,
      true,
    );
    assert.equal(
      manager.commitState("test/v1", 1, { version: 2 }).committed,
      true,
    );
    ...
    const recovered = new ExtensionStateManager(runtimeDir).loadState(
      "test/v1",
    );
