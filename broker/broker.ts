...
class IntercomBroker {
  ...
  start(): void {
    ...
    if (typeof LISTEN_TARGET === "string") {
      ...
      this.server.listen(
        { host: LISTEN_TARGET.host, port: LISTEN_TARGET.port },
        onListening,
      );
    }
  ...
  private handleMessage(
    ...
    if (
      requiresEndpointAuth &&
      clientMessage.type === "register" &&
      !hasEndpointAuth
    ) {
      throw new Error("Invalid intercom TCP endpoint credentials");
    }
    ...
    switch (clientMessage.type) {
      case "register": {
        ...
        if (extensions !== undefined) {
          if (
            ...
            extensions.length > MAX_EXTENSIONS_PER_SESSION
          ) {
            throw new Error(
              `Invalid extensions field (maximum ${MAX_EXTENSIONS_PER_SESSION})`,
            );
          }
          for (const extension of extensions) {
            if (!this.validateExtensionCapability(extension)) {
              throw new Error(
                `Invalid extension capability: ${JSON.stringify(extension)}`,
              );
            }
          }
        }
        ...
        if (extensions) {
          for (const ext of extensions) {
            ...
            writeMessage(socket, {
              ...
              ...(owner
                ? { ownerId: owner.sessionId, ownerEpoch: owner.epoch }
                : {}),
            });
      ...
      case "unregister": {
        ...
        if (existing?.socket === socket) {
          ...
          this.broadcast(
            { type: "session_left", sessionId: currentId },
            currentId,
          );
          this.recomputeNamespaceOwners();
          this.scheduleShutdownCheck();
      ...
      case "extension_capabilities_update": {
        if (!currentId) {
          throw new Error(
            "Received extension_capabilities_update before register",
          );
        }
        ...
        for (const extension of extensions) {
          ...
          const state = this.extensionStateManager.loadState(
            extension.namespace,
          );
          if (state) {
            writeMessage(socket, {
      ...
      case "send": {
        ...
        const replyEdge = message.replyTo
          ? this.askEdges.get(message.replyTo)
          : undefined;
        ...
        if (targets.length === 1) {
          ...
          if (message.supersedes) {
            const supersededRoute = this.messageReceiptRoutes.get(
              message.supersedes,
            );
            if (
              !supersededRoute ||
              supersededRoute.from !== currentId ||
              supersededRoute.to !== target.info.id
            ) {
              writeMessage(socket, {
                type: "delivery_failed",
                messageId: message.id,
          ...
          if (
            replyEdge &&
            (replyEdge.to !== currentId || replyEdge.from !== target.info.id)
          ) {
            writeMessage(socket, {
              type: "delivery_failed",
              messageId: message.id,
        ...
        const disconnectedTargets = this.findDisconnectedSessions(
          clientMessage.to,
        );
        if (disconnectedTargets.length === 1) {
          ...
          if (
            replyEdge &&
            (replyEdge.to !== currentId || replyEdge.from !== target.id)
          ) {
            writeMessage(socket, {
              type: "delivery_failed",
              messageId: message.id,
          ...
          const liveMailboxTarget =
            this.findUniqueLiveSessionForDisconnectedSession(target, currentId);
      ...
      case "message_receipt": {
        ...
        const route = this.messageReceiptRoutes.get(
          clientMessage.receipt.messageId,
        );
      ...
      case "cancel_message": {
        ...
        const queuedIndex = this.mailboxMessages.findIndex(
          (entry) =>
            entry.message.id === clientMessage.messageId &&
            entry.from.id === currentId,
        );
        ...
        if (session?.socket === socket) {
          ...
          if (clientMessage.runtimeFallbackAlias !== undefined) {
            ...
            if (
              ...
              clientMessage.runtimeFallbackAlias
            ) {
              session.info.runtimeFallbackAlias =
                clientMessage.runtimeFallbackAlias;
              changed = true;
          ...
          if (
            changed ||
            now - session.lastPresenceBroadcastAt >= PRESENCE_HEARTBEAT_MS
          ) {
  ...
  private rememberDisconnectedSession(
    info: SessionInfo,
    now = Date.now(),
  ): void {
  ...
  private flushMailboxForSession(
    session: ConnectedSession,
    now = Date.now(),
  ): void {
    this.pruneMailboxMessages(now);
    const sessionName = session.info.name?.toLowerCase();
    const uniqueMailboxIdentity =
      this.findLiveSessionsSharingMailboxIdentity(session.info).length === 1;
  ...
  private findUniqueLiveSessionForDisconnectedSession(
    info: SessionInfo,
    senderId?: string,
  ): ConnectedSession | null {
    const matches = this.findLiveSessionsSharingMailboxIdentity(info).filter(
      (session) => session.info.id !== senderId,
    );
   ...
   */
  private findLiveSessionsSharingMailboxIdentity(
    info: SessionInfo,
  ): ConnectedSession[] {
  ...
  private validateExtensionCapability(
    cap: unknown,
  ): cap is ExtensionCapability {
    ...
    if (
      ...
      typeof c.ownerEligible !== "boolean"
    ) {
      return false;
    }
    ...
    for (const namespace of namespaces) {
      ...
      if (candidates.length === 0) {
        if (this.namespaceOwners.delete(namespace)) {
          for (const session of this.sessions.values()) {
            const isCapable = session.extensions?.some(
      ...
      const socketChanged =
        existing && existing.socket !== winner.session.socket;
  ...
  private handleExtensionPublish(
    ...
    const hasCapability = session.extensions?.some(
      (ext) => ext.namespace === namespace,
    );
    if (!hasCapability) {
      writeMessage(socket, {
    ...
    for (const [recipientId, recipientSession] of this.sessions) {
      ...
      const isCapable = recipientSession.extensions.some(
        (ext) => ext.namespace === namespace,
      );
      if (!isCapable) {
        continue;
  ...
  private handleExtensionStateCommit(
    ...
    const result = this.extensionStateManager.commitState(
      namespace,
      expectedRevision,
      payload,
    );
