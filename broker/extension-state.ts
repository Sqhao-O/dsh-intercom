...
function serializePayload(payload: unknown): string | null {
  try {
    ...
    if (
      json === undefined ||
      Buffer.byteLength(json, "utf8") > MAX_STATE_BYTES
    ) {
      return null;
    }
    return json;
...
export class ExtensionStateManager {
  private readonly states = new Map<
    string,
    { revision: number; payload: unknown }
  >();
  ...
  loadState(namespace: string): { revision: number; payload: unknown } | null {
    ...
    const envelope =
      this.readEnvelope(this.statePath(namespace), namespace) ??
  ...
  commitState(
    ...
    try {
      ...
      // Opened "r+" (not "r"): fsync on a read-only handle fails with EPERM on
      // Windows; a writable handle fsyncs the same data on every platform.
