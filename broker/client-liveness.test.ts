 ...
 * intercom roster forever (this is why long-lived headless/RPC dsh sessions
...
const homeDir = mkdtempSync(path.join(tmpdir(), "dsh-intercom-liveness-unit-"));
const runtimeAgentDir =
  process.platform === "win32" ? undefined : mkdtempSync("/tmp/dshic-");
const previousHome = process.env.HOME;
const previousUserProfile = process.env.USERPROFILE;
const previousAgentDir = process.env.DSH_HOME;
process.env.HOME = homeDir;
process.env.USERPROFILE = homeDir;
if (runtimeAgentDir) process.env.DSH_HOME = runtimeAgentDir;
process.env.DSH_INTERCOM_LIVENESS_INTERVAL_MS = "100";
process.env.DSH_INTERCOM_LIVENESS_TIMEOUT_MS = "200";
...
async function registeredClientAgainstFakeSocket(): Promise<{
  ...
  const server = net.createServer((serverSide) => {
    serverSide.on("data", (data: Buffer) => {
      ...
      try {
        ...
        if (json.type === "register") {
          writeMessage(serverSide, {
            type: "registered",
            sessionId: json.sessionId ?? "stable-test",
            features: [],
          });
        }
        if (json.type === "list") {
...
test("client emits disconnected when the peer destroys the socket without a FIN", async () => {
  const { client, closeServerSideAbruptly } =
    await registeredClientAgainstFakeSocket();
  try {
    assert.equal(
      client.isConnected(),
      true,
      "client should be connected after register",
    );
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () =>
            reject(
              new Error(
                "client never noticed the abruptly-closed peer (half-open socket)",
              ),
            ),
          3000,
        ),
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () =>
            reject(
              new Error(
                "liveness heartbeat did not detect the half-open socket within 3s",
              ),
            ),
          3000,
        ),
