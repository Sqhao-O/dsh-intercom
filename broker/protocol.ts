...
export function isMessage(value: unknown): value is Message {
  ...
  return (
    ...
    (Array.isArray(value.content.attachments) &&
      value.content.attachments.every(isAttachment))
  );
}
...
export function isSessionInfo(value: unknown): value is SessionInfo {
  ...
  if (
    value.runtimeFallbackAlias !== undefined &&
    typeof value.runtimeFallbackAlias !== "boolean"
  ) {
    return false;
  }
  ...
  return (
    value.trustedLocal === undefined || typeof value.trustedLocal === "boolean"
  );
}
...
export function isSessionRegistration(
  value: unknown,
): value is SessionRegistration {
  if (!isRecord(value)) {
    return false;
  }
