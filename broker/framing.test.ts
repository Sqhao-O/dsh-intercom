...
test("createMessageReader handles normal fragmented frames", () => {
  ...
  const frameA = framePayload(
    Buffer.from(JSON.stringify({ type: "one" }), "utf-8"),
  );
  const frameB = framePayload(
    Buffer.from(JSON.stringify({ type: "two" }), "utf-8"),
  );
...
test("createMessageReader reassembles a fragmented frame after a header-boundary fast-path frame", () => {
  ...
  const frameB = framePayload(
    Buffer.from(JSON.stringify({ bb: "1234567890" }), "utf-8"),
  );
...
test("createMessageReader rejects an oversized declared frame", () => {
  ...
  const oversizedFrame = framePayload(
    Buffer.from(JSON.stringify({ text: "too large" }), "utf-8"),
  );
  ...
  assert.match(
    errors[0].message,
    /Intercom frame length \d+ exceeds maximum 8 bytes/,
  );
});
...
test("createMessageReader rejects an oversized frame before retaining same-chunk payload bytes", () => {
  ...
  assert.equal(
    errors[0].message,
    "Intercom frame length 9 exceeds maximum 8 bytes",
  );
});
