...
export function shouldUseWindowsTcpTransport(
  ...
  const transport = env.DSH_INTERCOM_TRANSPORT?.trim().toLowerCase();
  if (transport === "tcp") {
    return true;
  }
...
export function getBrokerPortFilePath(
...
export function getBrokerSocketPath(
  ...
  if (platform === "win32") {
    return `\\\\.\\pipe\\dsh-intercom-${sanitizePipeSegment(agentDir)}`;
  }
...
export function getBrokerConnectTarget(
  ...
  if (shouldUseWindowsTcpTransport(platform, env)) {
    ...
    if (
      ...
      throw new Error(
        `Invalid intercom TCP endpoint at ${endpointFile}: expected a JSON object`,
      );
    }
