...
function getLivenessIntervalMs(): number {
  const raw = Number.parseInt(
    process.env.DSH_INTERCOM_LIVENESS_INTERVAL_MS ?? "",
    10,
  );
...
function getLivenessTimeoutMs(): number {
  const raw = Number.parseInt(
    process.env.DSH_INTERCOM_LIVENESS_TIMEOUT_MS ?? "",
    10,
  );
  return Number.isFinite(raw) && raw > 0
    ? Math.min(raw, getLivenessIntervalMs())
    : 5_000;
}
...
export class IntercomClient extends EventEmitter {
  ...
  private pendingLists = new Map<
    string,
    { resolve: (sessions: SessionInfo[]) => void; reject: (e: Error) => void }
  >();
    ...
    return new Promise((resolve, reject) => {
      ...
      const onClose = () => {
        ...
        const disconnectError =
          this.disconnectError ?? new Error("Client disconnected");
      ...
      const onReaderError = (error: Error) => {
        const protocolError = new Error(
          `Intercom protocol error: ${error.message}`,
          { cause: error },
        );
        if (!connectionEstablished) {
    ...
    switch (brokerMessage.type) {
      case "registered": {
        ...
        this._features = new Set(
          (brokerMessage.features as string[] | undefined) ?? [],
        );
      ...
      case "message_receipt": {
        if (
          !isSessionInfo(brokerMessage.from) ||
          !isMessageReceipt(brokerMessage.receipt)
        ) {
      ...
      case "message_control": {
        if (
          !isSessionInfo(brokerMessage.from) ||
          !isMessageControl(brokerMessage.control)
        ) {
      ...
      case "extension_state_result": {
        if (
          ...
          !Number.isSafeInteger(brokerMessage.revision) ||
          Number(brokerMessage.revision) < 0 ||
          (brokerMessage.reason !== undefined &&
            typeof brokerMessage.reason !== "string")
        ) {
  ...
  updateExtensionCapabilities(
    extensions: SessionRegistration["extensions"],
  ): void {
  ...
  sendExtensionMessage(
    message: Extract<
      ClientMessage,
      { type: "extension_publish" | "extension_state_commit" }
    >,
  ): void {
    if (!this.supportsFeature(EXTENSION_BUS_FEATURE)) {
      throw new Error(
        `Connected broker does not support ${EXTENSION_BUS_FEATURE}`,
      );
    }
    const socket = this.requireActiveSocket();
