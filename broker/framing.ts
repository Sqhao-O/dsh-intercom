...
export function createMessageReader(
  ...
  function reportMessage(framePayload: Buffer): boolean {
    ...
    try {
      ...
      onError(
        new Error(`Failed to parse intercom message: ${message}`, {
          cause: error,
        }),
      );
      return false;
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      onError(
        new Error(`Failed to handle intercom message: ${message}`, {
          cause: error,
        }),
      );
      return false;
    ...
    while (offset < data.length) {
      if (headerBytes < 4) {
        ...
        if (payloadLength > maxFrameBytes) {
          ...
          onError(
            new Error(
              `Intercom frame length ${payloadLength} exceeds maximum ${maxFrameBytes} bytes`,
            ),
          );
          return;
        }
