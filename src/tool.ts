/**
 * The `intercom` tool definition: one tool, multiple actions. M1 implements
 * `list` / `send` / `status` / `name`; `ask` / `reply` / `pending` / `cancel`
 * arrive with the broker transport in M2.
 */
import type { Agent } from "@deepseek-ai/dsh-agent";
import { defineTool } from "@deepseek-ai/dsh-tools";
import type { ToolDefinition } from "@deepseek-ai/dsh-tools";
import { formatSessionList, formatSessionListRow } from "./message.ts";
...
export function createIntercomTool(deps: IntercomToolDeps): ToolDefinition {
  return defineTool({
    name: "intercom",
    description:
      "Exchange messages with other live dsh sessions in this process. Use to coordinate with peer sessions: " +
      "list them, give the current session a name, or send a message. Address sessions by alias, full session id, " +
      'or the short id prefix shown in parentheses by "list". The message arrives as a relay from the named sender: ' +
      "idle sessions start a new turn with it, busy sessions receive it as steering at the next step boundary.",
    parameters: {
      action: {
        type: "string",
        enum: ["list", "send", "status", "name"],
        required: true,
        description:
          "'list': show live sessions. 'send': deliver a message to another session. 'name': set the current session's intercom alias. 'status': intercom plugin status.",
      },
      to: {
        type: "string",
        description:
          "Target session for 'send': alias, full session id, or the unique id prefix shown in parentheses by 'list'.",
      },
      message: {
        type: "string",
    ...
    execute: async (args, exec) => {
      ...
      switch (args.action) {
        ...
        case "send": {
          ...
          if (!to)
            throw new Error(
              'intercom: "send" requires a "to" parameter (alias or session id).',
            );
          const body = args.message;
          if (typeof body !== "string" || !body.trim()) {
            throw new Error(
              'intercom: "send" requires a non-empty "message" parameter.',
            );
          }
        ...
        default:
          throw new Error(
            `intercom: unknown action ${JSON.stringify(args.action)}. Supported actions: list, send, status, name.`,
          );
      }
    },
  });
