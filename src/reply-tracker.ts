...
function resolvePendingSender(
  ...
  const exactNameMatches = pending.filter(
    (context) => context.from.name?.toLowerCase() === lowerTo,
  );
  ...
  const idPrefixMatches = pending.filter((context) =>
...
export class ReplyTracker {
   ...
   */
  activateTurnContext(messageId: string, now = Date.now()): void {
    this.pruneExpired(now);
  ...
  resolveReplyTarget(
    ...
    if (options.replyTo) {
      ...
      if (options.to && !matchesPendingSender(target, options.to)) {
        throw new Error(
          `Pending ask "${options.replyTo}" is not from "${options.to}"`,
        );
      }
      return target;
    }
  ...
  findUniquePendingAskFrom(
    to: string,
    now = Date.now(),
  ): IntercomContext | null {
    const candidates = Array.from(this.pendingAsks.values()).filter(
      (context) => {
        ...
        return (
          context.from.id === to ||
          context.from.name?.toLowerCase() === to.toLowerCase()
        );
      },
    );
  ...
  dismissPendingAsk(replyTo: string): void {
    ...
    for (
      let index = this.pendingTurnContexts.length - 1;
      index >= 0;
      index -= 1
    ) {
